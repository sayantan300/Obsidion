...
class PythonNews(Cog):
    ...
    async def start_tasks(self) -> None:
        """Start the tasks for fetching news from minecraft"""
    ...
    async def post_pep_news(self) -> None:
        ...
        for new in data["entries"]:
            ...
            if msg.channel.is_news():
                log.trace(
                    "Publishing PEP annnouncement because it was in a news channel"
                )
                await msg.publish()
        ...
        for maillist in constants.PythonNews.mail_lists:
            ...
            for thread in recents.html.body.div.find_all("a", href=True):
                ...
                msg = await self.send_webhook(
                    ...
                    description=content[:500] + f"... [continue reading]({link})"
                    if len(content) > 500
                    else content,
                    timestamp=new_date,
                    url=link,
                    author=f"{email_information['sender_name']} ({email_information['sender']['address']})",
                    author_url=MAILMAN_PROFILE_URL.format(
    ...
    async def send_webhook(
        ...
        return await self.webhook.send(
            embed=embed, username=webhook_profile_name, avatar_url=AVATAR_URL, wait=True
        )
